...
from utils import compute_mesh_diameter, erode_depth, bilateral_filter_depth, set_seed, make_mesh_tensors, toOpen3dCloud, sample_views_icosphere, euler_matrix, depth2xyzmap_batch, dr, mycpp
...
class FoundationPose:
    def __init__(self, model_pts, model_normals, symmetry_tfs=None, mesh=None, scorer: ScorePredictor = None, refiner: PoseRefinePredictor = None, glctx=None):
        self.gt_pose = None
    ...
    def reset_object(self, model_pts, model_normals, symmetry_tfs=None, mesh=None):
        ...
        self.mesh = mesh.copy()
        self.mesh.vertices = self.mesh.vertices - self.model_center.reshape(1, 3)
        self.mesh_tensors = make_mesh_tensors(self.mesh)

        model_pts = self.mesh.vertices
        self.diameter = compute_mesh_diameter(model_pts=self.mesh.vertices, n_sample=10000)
    ...
    def register(self, K, rgb, depth, ob_mask, ob_id=None, glctx=None, iteration=5):
        ...
        xyz_map = depth2xyzmap(depth, K)
        poses, vis = self.refiner.predict(mesh=self.mesh, mesh_tensors=self.mesh_tensors, rgb=rgb, depth=depth, K=K, ob_in_cams=poses.data.cpu(
        ).numpy(), normal_map=normal_map, xyz_map=xyz_map, glctx=self.glctx, mesh_diameter=self.diameter, iteration=iteration, get_vis=self.debug >= 2)
        if vis is not None:
            imageio.imwrite(f'{self.debug_dir}/vis_refiner.png', vis)

        scores, vis = self.scorer.predict(mesh=self.mesh, rgb=rgb, depth=depth, K=K, ob_in_cams=poses.data.cpu().numpy(
        ), normal_map=normal_map, mesh_tensors=self.mesh_tensors, glctx=self.glctx, mesh_diameter=self.diameter, get_vis=self.debug >= 2)
        if vis is not None:
    ...
    def track_one(self, rgb, depth, K, iteration, extra={}):
        ...
        xyz_map = depth2xyzmap_batch(depth[None], torch.as_tensor(
            K, dtype=torch.float, device='cuda')[None], zfar=np.inf)[0]

        pose, vis = self.refiner.predict(mesh=self.mesh, mesh_tensors=self.mesh_tensors, rgb=rgb, depth=depth, K=K, ob_in_cams=self.pose_last.reshape(
            1, 4, 4).data.cpu().numpy(), normal_map=None, xyz_map=xyz_map, mesh_diameter=self.diameter, glctx=self.glctx, iteration=iteration, get_vis=self.debug >= 2)
